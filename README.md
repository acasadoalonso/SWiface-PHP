...
`unobscure()`. Both generated files are git-ignored; `config.bkup` is a sample of the result
and `configtail.template` is a sample of the `configtail.txt` tail (accent-stripping helper
plus the fallback event/turnpoint/track definitions used when no JSON file is found).
...
- `config.bkup` is a committed *sample* configuration that contains a real-looking GitHub
  token and obscured DB passwords. Rotate anything still in use and prefer a redacted sample.
