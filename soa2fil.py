	...
	url3=getlinks(cl, "class_results")      # search for the contestants on each class
        ...
        for ft in fft:
                ...
                if "igc_file" in ft:
                    ...
                    print ">>> missing >>>>>", pil["first_name"], pil["last_name"]
                    continue
                igcfilename="PAT/"+igcfile[0:3]+"/"+classname+"-"+igcfile[4:]
                if not os.path.isdir("PAT/"+igcfile[0:3]):
                    os.system("mkdir PAT/"+igcfile[0:3])
                    print " OK directory made"
                print "FFTC", igcfilename, fftc,pil["first_name"], pil["last_name"], pil['nationality']
